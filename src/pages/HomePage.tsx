...
export const HomePage = () => {
  ...
  const handlePrev = () => setLightboxPhoto(FEATURED[(lightboxIdx - 1 + FEATURED.length) % FEATURED.length] ?? null)
  const handleNext = () => setLightboxPhoto(FEATURED[(lightboxIdx + 1) % FEATURED.length] ?? null)
    ...
    <Box>
      ...
      >
        ...
        <Container maxWidth="md" sx={{ textAlign: 'center', position: 'relative', zIndex: 1 }}>
          ...
          >
            Tortas artesanales, desayunos sorpresa, cupcakes, alfajores y figuras en porcelana fría. Cada creación, un
            regalo para los sentidos.
          </Typography>
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ justifyContent: 'center' }}>
            <Button component={Link} to="/galeria" variant="contained" size="large" sx={{ px: 4, py: 1.5 }}>
              Ver galería
            </Button>
            <Button component={Link} to="/contacto" variant="outlined" size="large" sx={{ px: 4, py: 1.5 }}>
              Hacer un pedido
            </Button>
          </Stack>
        </Container>
      ...
      <Box
        sx={{
          bgcolor: theme.palette.mode === 'light' ? `${theme.palette.primary.light}22` : theme.palette.background.paper,
          py: 8,
        }}
      >
        <Container maxWidth="lg">
          <Typography variant="h3" sx={{ mb: 1, fontFamily: "'Playfair Display', serif", textAlign: 'center' }}>
            Nuestros productos
          </Typography>
          <Typography
            variant="body1"
